...
import os
import config
from config import MNEMONIC_PHRASE,PROJECT_WALLET_ADDRESS


def send_radix(recipient_address, transaction_message):
    ...
    transfer_amount: int = radix.derive.atto_from_xrd(1)  # We will be sending them 200 XRD.
    ...
    # string.
    mnemonic_phrase: str = os.environ[f'{MNEMONIC_PHRASE}']
...
def send_junger_token(recipient_address, transaction_message):
    ...
    transfer_amount: int = radix.derive.atto_from_xrd(1)
    ...
    tx_hash: str = wallet.build_sign_and_send_transaction(
        actions=(
            ...
            .token_transfer(
                from_account_address=recipient_address,
                to_account_address=PROJECT_WALLET_ADDRESS,
                token_rri=token_rri,
                transfer_amount=transfer_amount
            )
        ),
